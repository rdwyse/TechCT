...
columns_to_drop_1_T1 = ['SelfMonitoring3', 'SelfMonitoring4', 'SelfMonitoring5', 'ConfMonitoring3', 'ConfMonitoring4', 'ConfMonitoring5']
...
def calculate_kappa(df, conf_rater_columns, self_rater_columns):
    ...
    for conf_col, self_col in zip(conf_rater_columns, self_rater_columns):
        ...
        if len(rater1) > 0 and len(rater2) > 0:
            ...
            results.append([f"{df['StudyPhase'].iloc[0]} - {conf_col} vs {self_col}", kappa, percent_agreement * 100, num_observations, true_positive, false_positive, false_negative, true_negative, total_agreement_count, total_observation_count, expected_agreement])
    ...
    if len(rater1_all) > 0 and len(rater2_all) > 0:
        ...
        results.append([f"{df['StudyPhase'].iloc[0]} - Entire Set", kappa, percent_agreement * 100, num_observations, true_positive, false_positive, false_negative, true_negative, total_agreement_count, total_observation_count, expected_agreement])
