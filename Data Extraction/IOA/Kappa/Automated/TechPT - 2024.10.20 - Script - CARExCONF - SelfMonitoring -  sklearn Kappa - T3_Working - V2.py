...
if len(rater1_all) > 0 and len(rater2_all) > 0:
    ...
    results = []
    results.append(["Entire Set", kappa, percent_agreement * 100, num_observations, true_positive, false_positive, false_negative, true_negative, total_agreement_count, total_observation_count, expected_agreement])
...
for conf_col, self_col in zip(conf_rater_columns, self_rater_columns[:len(conf_rater_columns)]):
    ...
    if len(rater1) > 0 and len(rater2) > 0:
        ...
        results.append([f"{conf_col} vs {self_col}", kappa, percent_agreement * 100, num_observations, true_positive, false_positive, false_negative, true_negative, total_agreement_count, total_observation_count, expected_agreement])
...
for (caregiver_id, study_phase), group in df.groupby(['CaregiverID', 'StudyPhase']):
    ...
    if len(rater1_all) > 0 and len(rater2_all) > 0:
        ...
        results.append([f"CaregiverID {caregiver_id}, StudyPhase {study_phase}", kappa, percent_agreement * 100, num_observations, true_positive, false_positive, false_negative, true_negative, total_agreement_count, total_observation_count, expected_agreement])
